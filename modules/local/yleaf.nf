process YLEAF {
    ...
    script:
    // NOTE: the chrY subset MUST NOT be named <meta.id>.bam. Nextflow stages the input
    // alignment as a symlink under its own name, and when the analysis alignment is a BAM
    // rather than a CRAM (the Parabricks path, or any --skip_bqsr run) that symlink is
    // already called <meta.id>.bam. `samtools view -o <meta.id>.bam` then opens the output
    // through the symlink and truncates the upstream BAM in its own work directory --
    // destroying the aligner's output and every downstream task that reads it.
    //
    ...
    [ -n "\$YCHR" ] || YCHR=chrY
    samtools view -@ ${task.cpus} -b -T ${fasta} -o ${meta.id}.chrY.bam ${cram} "\$YCHR"
    samtools index -@ ${task.cpus} ${meta.id}.chrY.bam
...
CFG
    ...
    PYTHONPATH="\$PWD" python3 -m yleaf.Yleaf \\
        -bam ${meta.id}.chrY.bam -rg hg38 -o ${meta.id}_yleaf -t ${task.cpus} -force \\
    ...
    rm -f ${meta.id}.chrY.bam ${meta.id}.chrY.bam.bai
